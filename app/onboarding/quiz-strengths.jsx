...
export default function QuizStrengthsScreen() {
  const router = useRouter();
  const { quizAnswers, setQuizAnswer } = useAppStore();
  const questions = QUIZ_QUESTIONS.strengths;
  ...
  const handleSelect = (optionIndex) => {
    ...
    if (currentQ < questions.length - 1) {
      ...
      router.push('/onboarding/quiz-interests');
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-white">
      <View className="px-6 pt-4 pb-2">
        <View className="flex-row justify-between items-center mb-4">
          ...
          <Text className="text-gray-400 text-sm">Section 2 of 4</Text>
        </View>

        <ProgressBar
          current={currentQ + 1}
          total={questions.length}
          label="Strengths Inventory"
        />
